...
$trajetC->updateTrajet($_GET['id'],$trajet);
header('location: trajetConducteur.php'); 

}
$trajet=$trajetC->findTrajetById($_GET['id']);
...
<a href="trajetConducteur.php" style="padding: 8px 12px;background-color: red;color: #fff;border: none;border-radius: 5px;cursor: pointer;transition: background-color 0.3s; margin-top:10px;">back</a>
    ...
    <form action="" method="POST" id="trajetForm">
        <div class="form-group">
            ...
            <input type="text" id="departure" name="departure" value="<?php echo $trajet['departure'];?>">
            <span class="error" id="errorDeparture"></span>
        </div>
        <div class="form-group">
            <label for="destination">Destination:</label>
            <input type="text" id="destination" name="destination" value="<?php echo $trajet['destination'];?>">
            <span class="error" id="errorDestination"></span>
        </div>
        <div class="form-group">
            <label for="date">Date:</label>
            <input type="date" id="date" name="date" value="<?php echo $trajet['date'];?>">
            <span class="error" id="errorDate"></span>
        </div>
        <div class="form-group">
            <label for="price">Price:</label>
            <input type="number" id="price" name="price" step="0.01" value="<?php echo $trajet['price'];?>">
            <span class="error" id="errorPrice"></span>
        </div>
        <div class="form-group">
            <label for="nb_place">Number of Places:</label>
            <input type="number" id="nb_place" name="nb_place" min="1" value="<?php echo $trajet['nb_place'];?>">
            <span class="error" id="errorNbPlace"></span>
        </div>
        <button type="submit" class="submit-btn">modifier Trajet</button>
    </form>
</div>
